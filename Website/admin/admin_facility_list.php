...
if(isset($_SESSION['admin']) && isset($_SESSION['isauth'])){
    ...
    if(isset($_GET['type']) && $_GET['type']!=''){
    	...
    	if($type=='delete'){
    	    $id=$_GET['id'];
            $delete_sql="delete from facilities where fac_id='$id'";
            mysqli_query($conn,$delete_sql);
    	}
    }
    $sql="select * from facilities order by fac_id asc";
                      ...
                      <div class="" style="display:flex;">
                        ...
                        <div class="container">
            		        <div class="col-12 col-md-12 col-sm-12 py-1" style="display:inline-block;">
                                <button type="button" 
                                   onclick="window.location.replace('./admin_facility_list.php?logout=1');"
                                    style="float:right;" class="btn btn-outline-light">Logout
                                </button>
                                <button type="button" 
                                   onclick="window.location.replace('./admin_facility_add.php');"
                                    style="float:right;" class="btn btn-outline-light">Add Facility
                                </button>
                        	</div>
            	        </div>
                      </div>
            	...
            	<div class="orders">
            	   <div class="row">
            		  <div class="col-xl-12 col-md-12">
            			 <div class="card">
            				<div class="card-body--">
            				   <div class="table-stats order-table ov-h" style="height:60vh;overflow-y:scroll;">
            					  <table class="table ">
            						 <thead>
            							<tr>
            							   <th>ID</th>
            						 ...
            						 <tbody>
            							...
            							<tr>
            							   ...
            							   <td><?php echo $row['fac_city']?></td>
            							   <td><?php echo $row['fac_address']?></td>
            							   <td><?php echo $row['fac_contact']?></td>
            							   <td><?php echo $row['beds_occupacy']?></td>
            							   <td style="display: flex; flex-flow:column;">
            								<?php
            								// if($row['status']==1){
            								// 	echo "<span class='badge badge-complete'><a href='?type=status&operation=deactive&id=".$row['user_id']."'>Active</a></span>&nbsp;";
            								// }else{
            								// 	echo "<span class='badge badge-pending'><a href='?type=status&operation=active&id=".$row['user_id']."'>Deactive</a></span>&nbsp;";
            								// }
            								echo "<span class='badge badge-edit'><a href='admin_facility_edit.php?id=".$row['fac_id']."'>Edit</a></span>&nbsp;";
            								
            								echo "<span class='badge badge-delete'><a href='?type=delete&id=".$row['fac_id']."'>Delete</a></span>";
